...
int main() {
  srand(seed);
  auto tr=gen_tree<2*D, H, true>(N, true);
  u64 found = 0;
  ...
  std::string file(std::string("intersect_query_simd") + std::string("_") + std::to_string(D) + std::string("_") + std::to_string(H) + std::string("_") + std::to_string(N));
  std::ofstream fout(std::string("benchmarks/intersect_query_simd/") + file);
  fout << int(s/k);
